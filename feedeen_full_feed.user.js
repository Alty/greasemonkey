...
// Based on "Feedly Full Feed"(https://userscripts.org/scripts/show/171868) Thanks fxwiki.
...
/*
  ...
  */
  ...
  '.fd_itemlist .entryBody .clear { font-size: inherit !important; height: auto !Important; width: auto !important; }',
//  '.u100Entry .title { display: inline !important; }',
  '.gm_fullfeed_loading {  }',
...
//  this.itemInfo.item_body = getFirstElementByXPath(returnXpathEntry() + '//div[contains(concat(" ", @class, " "), " entryBody ")]');
  this.itemInfo.item_body = returnXpathEntry.querySelector('iframe').parentNode;
...
//  var el = getFirstElementByXPath(returnXpathEntry() + '//span[contains(concat(" ", @class, " "), " gm_fullfeed_checked_icon ")]');
  var el = returnXpathEntry() .querySelector('gm_fullfeed_checked_icon'); // Feedeen
...
//    var e1 = $id('box');
    var e1 = $id('main-loading');	// Feedeen
//    var e2 = getFirstElementByXPath(returnXpathEntry());
    var e2 = returnXpathEntry(); // Feedeen
        ...
        if (nextEl && finalUrl !== nextEl.href) {
          ...
          if (flag === 'full') {
            ...
            if (!bCache) {
              ...
              try {
                localStorage.setItem('FeedeenFullFeed_userCacheAP', JSON.stringify(userCacheAP));	// Feedeen
                if (!bList) {
                  userCacheListAP.push(c.feed.url);
                  localStorage.setItem('FeedeenFullFeed_userCacheListAP', JSON.stringify(userCacheListAP)); // Feedeen
                }
              } catch(er) {}
            }
          }
  ...
  var getSiteinfo = function(data, flag) {
    data.forEach(function(ctx) {
      var opt = {
        ...
        url: ctx.url,
        headers: {
//		'User-Agent': navigator.userAgent + ' Greasemonkey (FeedlyFullFeed)'
			'User-Agent': navigator.userAgent + ' Greasemonkey (FeedeenFullFeed)' // Feedeen
        },
        onload: function(res) {
          if (res.status === 200) {
            if (flag) {
  ...
  if ($id('feedlyTabs')) {
    window.setTimeout(function() {
      ...
      if (div1) {
        var div2 = document.createElement('div');
        div2.className = 'tab';
        div2.innerHTML = '<div class="header target"><div id="gm_fullfeed_settings-menu" style="color:inherit; padding-left:32px;" target="new" class="label">Feedly Full Feed ' + loc[21] + '</div></div>';
        div1.appendChild(div2);
      }
    }, 10000);
  }
  */
  /*if ($id('settings-button-menu')) {
    ...
    if (d1) {
      ...
      d2.innerHTML = '<div id="gm_fullfeed_settings-menu2child" class="goog-menuitem-content">Feedly Full Feed ' + loc[21] + '</div>';
      d1.appendChild(d2);
    }
...
FullFeed.viewSettings = function(id) {
  ...
  try {
    title = returnXpathEntry().querySelector('a.fd_sitename').textContent; // Feedeen
  } catch(er) {}
  if (title) {
...
//  var container = getFirstElementByXPath(returnXpathEntry() + '//div[@class="entryHeader"]');
  var container = returnXpathEntry.querySelector('.fd_sitename').parentNode; // feedeen
  if (!container) return;
  itemSiteInfo = [];
...
//    var e = getElementsByXPath(returnXpathEntry() + '//div[@class="entryHeader"]/span[@class="gm_fullfeed_socialicon"]/a/img'), f = true;
    var e = returnXpathEntry().querySelector('span.gm_fullfeed_socialicon > a > img'), f = true;
    repeat ++;
    if (e) {
...
//  var exp = getFirstElementByXPath(returnXpathEntry() + '//div[@class="entryHeader"]/a');
  ...
  try {
    item.url =   returnXpathEntry().querySelector('a.fd_url').href;
    item.title = returnXpathEntry().querySelector('a.fd_title').innerHTML;
  } catch(e) {}
  return item;
};
...
//    var est = getFirstElementByXPath(returnXpathEntry() + '//a[@class="sourceTitle"]');
    if (est) {
      feed.url =  returnXpathEntry().querySelector('a.fd_siteurl').href;
      feed.title =  returnXpathEntry().querySelector('a.fd_sitename').innerHTML;
    }
  } catch(e) {}
  return feed;
};
...
//  this.item_container = getFirstElementByXPath(returnXpathEntry() + '//div[contains(concat(" ", @class, " "), " entryBody ")]');
  this.item_container = returnXpathEntry().querySelector('iframe').parentNode; // feedeen
  this.found = false;
};
...
//            var u = getElementsByXPath(returnXpathEntry() + '//div[contains(concat(" ", @class, " "), " entryBody ")]//a');
            var u = returnXpathEntry().querySelector('iframe > a'); // Feedeen : iframe内のaタグをどうやって拾おうか…。このままだと拾えない。TODOにしとく。
            if (u) excludeUrl(u);
          } catch(er) {}
        }
        break;
...
/*
  ...
  return (fo === 'T') ? 'id("timeline")//div[@class="inlineFrame"]'
	  ...
	  */
	//Feedeen
	//記事展開してれば、class=fd_itemかつexpand
	// FeedkyのinlineFrameは、Feedeenの.fd_item, .expand
...
//      el2 = getElementsByXPath(returnXpathEntry() + '//span[@class=" gm_fullfeed_checked"]', document);
      el2 = returnXpathEntry.querySelector('span.gm_fullfeed_checked');
      if (el2) {
